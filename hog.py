...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # loop / iteration to roll dice
    while loops < num_rolls:
        change = dice()
        total = change + total
        loops = loops + 1
        if change == 1:
...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    # x = tens digit of opp score
    # y = ones digit of player score
    # 451 % 10 = 1
    # 451 // 10 = 45 
    # ((451 // 10) % 10) = 5
    player = player_score % 10
    opp = ((opponent_score // 10) % 10)

    fart = 3 * (abs(opp - player))
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    # Number is sus if it has 3 or 4 factors
    # Suss fuss brings a number up to the next prime number if it has exactly 3 or 4 factors
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    # return a function and define the function
    def rolling(score0, score1):
        return n
    return rolling
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    # keep a variable that is equal to the return value of the strategy
    # while statment loop
    roll1 = strategy(0, 0)
    cp = 0
    while cp < goal:
...
def make_averaged(original_function, times_called=1000):
    ...
    def averaged(*arg):
        ...
        while n < times_called:
            average = original_function(*arg) + average
            n += 1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    sus_player_score = sus_update(0, score, opponent_score)
    return 0 if sus_player_score - score >= threshold else num_rolls
    # END PROBLEM 11
